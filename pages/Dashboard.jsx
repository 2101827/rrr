...
export default function Dashboard() {
  ...
  const getClientPaths = (client) => {
    ...
    return folders.flatMap(folder => [
      `/${folder}/charges.csv`, `/${folder}/denial.csv`, `/${folder}/openar.csv`, `/${folder}/aging.csv`,
    ]);
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    if (files.length !== 4) {
      setUploadError("Please select exactly 4 files: Charges, Denials, OpenAR, and Aging.");
    ...
    const processFile = (file) => {
      return new Promise((resolve) => {
        Papa.parse(file, {
          ...
          skipEmptyLines: true,
          complete: (results) => {
            // Normalize data (same as parseCSV logic)
            const normalized = results.data.map((r) => {
              const dos = r.Date_of_Service ? dayjs(r.Date_of_Service) : null;
              const ced = r.Charge_Entry_Date ? dayjs(r.Charge_Entry_Date) : null;
              return {
    ...
    try {
      ...
      parsedResults.forEach(({ name, data }) => {
        if (name.includes("charge")) {
          ...
          newDenials = data;
        } else if (name.includes("openar") || name.includes("open") || name.includes("ar")) {
          newOpenAR = data;
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        let allCharges = [], allDenials = [], allOpenAR = [], allAging = [];
        ...
        setCharges(allCharges); setDenials(allDenials); setOpenAR(allOpenAR); setAgingData(allAging);
      } catch (err) { console.error("Error loading CSV:", err); }
    };
    loadData();
  }, [selectedClient]);

  const { filteredCharges, filteredDenials, filteredOpenAR, filteredAging, prevCharges, prevDenials, prevOpenAR } = useMemo(() => {
  ...
  }, [charges, denials, openAR, agingData, startDate, endDate]);
  ...
  const calculateKPIs = (chargesData, denialsData, openARData) => {
    ...
    if (safeCharges.length === 0 && safeDenials.length === 0) {
        return { totalPayments: 0, totalClaims: 0, gcr: 0, ncr: 0, denialRate: 0, firstPassRate: 0, cleanClaimRate: 0, totalDenials: 0, totalOpenAR: 0 };
    }
    ...
    // --- FPR CALCULATION USES THE FIXED BOOLEAN ---
    ...
    const ncr = netBilled > 0 ? (totalPayments / netBilled) * 100 : 0;
  ...
  const kpiSparklines = useMemo(() => ({
    ...
    ncr: getMonthlyChargesTrend(rows => { const p = rows.reduce((a, r) => a + (r.Paid_Amount || 0), 0); const al = rows.reduce((a, r) => a + ((r.Billed_Amount || 0) - (r.Adjustment_Amount || 0)), 0); return al > 0 ? (p / al) * 100 : 0; }),
  ...
  const mainChartData = useMemo(() => {
    ...
    filteredDenials.forEach(r => {
          ...
          // Initialize if this month didn't exist in charges
          if (!map[m]) map[m] = { 
              paidSum: 0, billedSum: 0, adjustmentSum: 0, ccrSum: 0, ccrCount: 0, count: 0, 
          ...
          // SAFETY CHECK: Ensure counters exist (in case map was created by charges but missed keys)
          ...
          // Now this will work correctly because undefined is handled
    ...
    let chartData = Object.entries(map).sort((a, b) => a[1].date.unix() - b[1].date.unix()).map(([month, obj]) => {
        ...
        else if (selectedMetric === "NCR" && (obj.billedSum - obj.adjustmentSum) > 0) avgVal = (obj.paidSum / (obj.billedSum - obj.adjustmentSum)) * 100;
    ...
    return chartData;
  }, [filteredCharges, filteredDenials, selectedMetric, prevKPIs]);
  ...
  // --- UPDATED AR BUCKET LOGIC: SNAPSHOT OF END MONTH ---
  const arAgingPieData = useMemo(() => {
      const buckets = { "0-30 Days": 0, "31-60 Days": 0, "61-90 Days": 0, "90+ Days": 0 };
      
      // 1. Determine the target month from the selected "To Date" (endDate)
      // Example: If endDate is "2025-05-31", target is "May 25"
      const targetMonthStr = dayjs(endDate).format("MMM YY");

      // 2. Filter the RAW agingData for that specific month only
      // We do NOT use 'filteredAging' because that uses a Date Range (DOS).
      // We want a snapshot of the AR at the end of the period.
      const snapshotData = agingData.filter(row => row.month === targetMonthStr);

      // 3. Aggregate data for that specific month
  ...
  }, [agingData, endDate]); // Dependencies: Raw Data and the End Date
  ...
  const pyramidChartData = useMemo(() => {
      const prevKPIsDynamic = calculateKPIs(prevCharges, prevDenials, prevOpenAR);
      return [
  ...
  }, [currentKPIs, prevCharges, prevDenials, prevOpenAR]);
